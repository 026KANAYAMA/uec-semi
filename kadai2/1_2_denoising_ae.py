...
def main(rank, world_size):
    ...
    for epoch in range(epochs):
        ...
        model.train()
        total_loss = 0.0
        for noisy, img, label in data_loader:
            noisy, img = noisy.to(device), img.to(device)
            optimizer.zero_grad()
    ...
    # 保存
    torch.save(model.module.state_dict(), "./1_2_denoising_ae.pth")
